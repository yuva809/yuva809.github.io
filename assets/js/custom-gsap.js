...
(function ($) {
  ...
  // Respect the visitor's reduced-motion preference: nonessential
  // scroll-driven, text-split and parallax animation is skipped below.
  ...
  function smoothSctoll() {
    // Delegated on document (not bound directly to the links) because the
    // offcanvas nav is a clone injected via innerHTML after this runs —
    // a direct binding would attach to nodes that get thrown away.
    $(document).on("click", ".smooth a, a.smooth", function (event) {
      ...
      if (target.length) {
        ...
        if (smoother) {
          smoother.scrollTo(targetY, !prefersReducedMotion);
        } else {
          $("html, body")
            .stop()
            .animate({ scrollTop: targetY }, prefersReducedMotion ? 0 : 1500);
        }
      }
    });
  }
  ...
  ////////////////////////////////////////////////////
  // 03. Text Invart Js
  if (!prefersReducedMotion && $(".tw-itm-title tw-itm-anim").length) {
  ...
  if (!prefersReducedMotion && $(".tw-sub-tilte").length) {
  ...
  if (!prefersReducedMotion && $(".tw-itm-title").length) {
